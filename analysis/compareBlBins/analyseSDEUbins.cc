...
float kstest( int arr0[], int arr1[] ){
  ...
  float delBin = (maxrange-minrange)/10.;
  float cumul0[10];
  float cumul1[10];
...
int main (int argc, char *argv[]) {
  ...
  TH1F ksdist("ksdist", "Station "+nameStati, 100, 0, 1);
    ...
    for (unsigned int i = 0 ; i < event.Stations.size(); ++i){
      ...
      if ( event.Stations[i].IsUUB && event.Id != previusEvent && sameUtc != event.utctime() ) {
        ...
        if (event.Stations[i].Error==256) { //0+256
          ...
          //ksdist.Fill(event.utctime(), 0.5); //+kstest(mpmt1lbmeanf, mpmt1lbmeanl));
...
*/
  ...
  plain->SetPalette(53); 
  pmt1meanf.Draw("COLZ");
  c1.Print("../../plots/baselineMeanFirst100.pdf");
